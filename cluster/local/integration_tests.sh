...
docker tag "${BUILD_IMAGE}" ghcr.io/arielsepton/provider-sql:v1.1.2
docker push ghcr.io/arielsepton/provider-sql:v1.1.2


# "${UP}" xpkg xp-extract --from-daemon "${PACKAGE_IMAGE}" -o "${CACHE_PATH}/${PACKAGE_NAME}.gz" && chmod 644 "${CACHE_PATH}/${PACKAGE_NAME}.gz"


# up xpkg push ghcr.io/arielsepton/provider-sql:v1.1.2 -f _output/xpkg/linux_amd64/provider-helm-v0.0.0-238.g13f9d56.dirty.xpkg
...
docker tag "${CONTROLLER_IMAGE}" ghcr.io/arielsepton/provider-sql-controller:v1.1.2
docker push ghcr.io/arielsepton/provider-sql-controller:v1.1.2
